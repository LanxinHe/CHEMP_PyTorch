...
class CHEMPLayer(nn.Module):
    def __init__(self, length, sigma_square_v, input_size):
        ...
        self.delta = nn.Parameter(torch.ones([input_size]) * 0.33, requires_grad=True)
        ...
        self.w = nn.Parameter(torch.randn([length, 2*length]), requires_grad=True)
        self.b = nn.Parameter(torch.zeros([length]), requires_grad=True)
        self.lstm = nn.GRU(length, length, bidirectional=True, num_layers=5)
    ...
    def forward(self, inputs, p_pre):
        ...
        likelihood_tilde, _ = self.lstm(likelihood.permute(2, 0, 1))
        likelihood_tilde = torch.matmul(likelihood_tilde, self.w.T) + self.b
