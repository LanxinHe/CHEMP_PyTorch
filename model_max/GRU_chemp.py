...
class CHEMPLayer(nn.Module):
    def __init__(self, length, input_size, hidden_size, rnn_layers):
        ...
        self.linear_likelihood = nn.Linear(hidden_size, length)

        # self.w_delta = nn.Parameter(torch.randn([1, hidden_size]))
        # self.b_delta = nn.Parameter(torch.zeros([1]))
        self.linear_delta = nn.Linear(hidden_size, 1)
    ...
    def forward(self, inputs, p_pre, h_pre, sigma_square_v):
        ...
        likelihood_tilde = self.linear_likelihood(gru_outputs)
        ...
        delta = self.sigmoid(self.linear_delta(gru_outputs))
        ...
        p_tilde = self.softmax(likelihood_tilde.permute(1, 2, 0) - log_max)
